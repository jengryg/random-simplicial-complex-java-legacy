 ...
 *  Dependencies: Point2DGraham.java
...
public class GrahamScan {
    private Stack<Point2DGraham> hull = new Stack<Point2DGraham>();
    ...
    public GrahamScan(Point2DGraham[] points) {
        ...
        int k2;
        for (k2 = k1 + 1; k2 < n; k2++)
            if (Point2DGraham.ccw(a[0], a[k1], a[k2]) != 0) break;
        hull.push(a[k2 - 1]);    // a[k2-1] is second extreme point

        // Graham scan; note that a[n-1] is extreme point different from a[0]
        for (int i = k2; i < n; i++) {
            Point2DGraham top = hull.pop();
            while (Point2DGraham.ccw(hull.peek(), top, a[i]) <= 0) {
                top = hull.pop();
            }
            hull.push(top);
            hull.push(a[i]);
     ...
     */
    public Iterable<Point2DGraham> hull() {
        Stack<Point2DGraham> s = new Stack<Point2DGraham>();
        for (Point2DGraham p : hull) s.push(p);
        return s;
    }
    ...
    private boolean isConvex() {
        ...
        Point2DGraham[] points = new Point2DGraham[n];
        int k = 0;
        for (Point2DGraham p : hull()) {
            points[k++] = p;
        }

        for (int i = 0; i < n; i++) {
            if (Point2DGraham.ccw(points[i], points[(i + 1) % n], points[(i + 2) % n]) <= 0) {
                return false;
            }
        }
        return true;
